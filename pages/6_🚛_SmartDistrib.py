...
<div class="distrib-header">
    ...
    <p style="margin:0.3rem 0 0 0; opacity:0.8;">Pantau suhu, kelembaban, dan lokasi armada secara langsung. Cari rute terbaik otomatis.</p>
</div>
""", unsafe_allow_html=True)
...
with tab_route:
    # Load route icon as base64
    import base64 as _b64
    _route_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "assets", "route-optimization.png")
    try:
        with open(_route_path, "rb") as _rf:
            _route_b64 = _b64.b64encode(_rf.read()).decode()
        _route_img = f'<img src="data:image/png;base64,{_route_b64}" style="height:36px;vertical-align:middle;margin-right:8px;border-radius:8px;">'
    except:
        _route_img = '🗺️'
    
    st.markdown(f'<h3 style="display:flex;align-items:center;">{_route_img} AI Route Optimization — Algoritma Genetika</h3>', unsafe_allow_html=True)
    st.markdown("*Mencari rute terbaik dari ribuan kemungkinan untuk menekan BBM, waktu tempuh, dan risiko kerusakan kargo.*")
